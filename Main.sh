...
# 这里的变量主要是给默认值，实际安装时会被手动输入覆盖
export vless_port=${PORT:-443}
...
install_singbox() {
    ...
    # --- 端口配置 (手动输入，无检测) ---
    echo ""
    yellow "请手动配置端口 (直接回车将使用括号内的默认值)"
    
    reading "1. 请输入 VLESS-Reality 端口 (TCP) [默认: 443]: " input_vless
    export vless_port=${input_vless:-443}

    reading "2. 请输入 Hysteria2 端口 (UDP) [默认: 8443]: " input_hy2
    export hy2_port=${input_hy2:-8443}

    reading "3. 请输入 Tuic 端口 (UDP) [默认: 9443]: " input_tuic
    export tuic_port=${input_tuic:-9443}

    reading "4. 请输入 Nginx 订阅端口 (TCP) [默认: 80]: " input_sub
    export sub_port=${input_sub:-80}
    
    # 兼容原脚本逻辑，将nginx_port指向sub_port
    nginx_port=$sub_port
    # --- 配置结束 ---
...
cp /etc/nginx/nginx.conf /etc/nginx/nginx.conf.bak
    ...
    # 使用之前输入的 sub_port，如果未定义则默认为 80
    local port=${sub_port:-80}
